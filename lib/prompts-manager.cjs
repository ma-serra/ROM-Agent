...
const autoMigrate = require('./prompts-auto-migrate.cjs');
...
class PromptsManager {
  ...
  ensureDirectories() {
    // 🔧 v3.6.1: Try/catch robusto para evitar crash no Render
    try {
      ...
      console.log('✅ [PromptsManager] Diretórios criados com sucesso');
    } catch (error) {
      console.warn('⚠️  [PromptsManager] Erro ao criar diretórios de prompts:', error.message);
      console.warn('   Usando fallback em memória (prompts padrão)');
      // Sistema continua funcionando sem crashar
    }
  }
