...
async function extractWithPdfplumber(filePath) {
  return new Promise((resolve, reject) => {
    // 🔧 v3.6.2.2: LOGGING DEFENSIVO
    console.log(`🐍 [pdfplumber] ENTRADA: ${path.basename(filePath)}`);
    ...
    console.log(`🐍 [pdfplumber] Script encontrado: ${pythonScript}`);
    console.log(`🐍 [pdfplumber] Spawning python3...`);
    ...
    // 🔧 v3.6.2.2: TIMEOUT de 2 minutos para pdfplumber
    const PDFPLUMBER_TIMEOUT = 2 * 60 * 1000; // 2 minutos
    const timeoutId = setTimeout(() => {
      console.error(`❌ [pdfplumber] TIMEOUT após ${PDFPLUMBER_TIMEOUT/1000}s`);
      python.kill('SIGTERM');
      setTimeout(() => python.kill('SIGKILL'), 5000); // Force kill após 5s
      resolve({ success: false, error: `pdfplumber timeout após ${PDFPLUMBER_TIMEOUT/1000}s` });
    }, PDFPLUMBER_TIMEOUT);

    python.stdout.on('data', (data) => {
      stdout += data.toString();
      console.log(`🐍 [pdfplumber] stdout chunk: ${data.toString().slice(0, 100)}...`);
    });

    python.stderr.on('data', (data) => {
      stderr += data.toString();
      console.warn(`🐍 [pdfplumber] stderr: ${data.toString()}`);
    });

    python.on('close', (code) => {
      clearTimeout(timeoutId);
      const elapsed = Date.now() - startTime;
      console.log(`🐍 [pdfplumber] Processo encerrado: código ${code}, tempo ${elapsed}ms`);
      ...
      try {
        console.log(`🐍 [pdfplumber] Parsing JSON output (${stdout.length} bytes)...`);
        ...
        console.log(`🐍 [pdfplumber] SUCESSO - retornando resultado`);
        resolve(result);
      } catch (e) {
        console.error(`❌ [pdfplumber] Erro ao parsear JSON: ${e.message}`);
        console.error(`   stdout recebido: ${stdout.slice(0, 500)}`);
        resolve({ success: false, error: e.message });
      }
    });

    // 🔧 v3.6.2.2: Handler de erro do processo
    python.on('error', (err) => {
      clearTimeout(timeoutId);
      console.error(`❌ [pdfplumber] Erro ao spawnar processo: ${err.message}`);
...
async function extractPDF(filePath, onProgress = null) {
  // 🔧 v3.6.2.2: LOGGING DEFENSIVO
  console.log(`📋 [extractPDF] ENTRADA: ${path.basename(filePath)}`);
  ...
  console.log(`📋 [extractPDF] Tamanho: ${sizeMB.toFixed(2)} MB, isLarge: ${isLargePDF}`);
  ...
  // 1.5. Tentar pdfplumber (Python - estrutura de tabelas preservada)
  // ✨ NOVO: Extração estruturada com pdfplumber para tabelas e andamentos processuais
  if (!extractedText || isLargePDF) {
    // 🔧 v3.6.2.2: LOGGING DEFENSIVO antes de pdfplumber
    console.log(`📋 [extractPDF] Tentando pdfplumber...`);

    // 🔧 v3.6: Emitir progresso antes de chamar Python
    if (onProgress) {
      onProgress('Extraindo com pdfplumber (Python)...', 10);
    }

    const pdfplumberResult = await extractWithPdfplumber(filePath);
    console.log(`📋 [extractPDF] pdfplumber retornou: success=${pdfplumberResult.success}`);
    ...
    if (pdfplumberResult.success && pdfplumberResult.text && pdfplumberResult.text.length > 100) {
      ...
      // 🔧 v3.6: Emitir progresso após conclusão
      if (onProgress) {
        onProgress('pdfplumber concluído', 25);
      }
  ...
  if (!extractedText && CONFIG.extraction.usePdftotext) {
    try {
      // 🔧 v3.6.2.2: LOGGING DEFENSIVO antes de pdftotext
      console.log(`📋 [extractPDF] Tentando pdftotext...`);

      // 🔧 v3.6: Emitir progresso antes de chamar pdftotext
      if (onProgress) {
        onProgress('Extraindo com pdftotext...', 15);
      }
      ...
      console.log(`📋 [extractPDF] pdftotext retornou ${output ? output.length : 0} caracteres`);
      ...
      if (output && output.trim().length > 100) {
        ...
        // 🔧 v3.6: Emitir progresso após conclusão
        if (onProgress) {
          onProgress('pdftotext concluído', 25);
        }
      }
    } catch (e) {
  ...
  // 🔥 DETECÇÃO DE PDF ESCANEADO - Checagem de DENSIDADE (chars por página)
  if (extractedText) {
