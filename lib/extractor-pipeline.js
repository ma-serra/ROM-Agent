...
async function extractPDF(filePath) {
  ...
  if (extractedText) {
    ...
    // 🔥 FORÇA OCR se:
    // - Arquivo > 10 MB E ratio < 1% (PDF grande com pouco texto = escaneado)
    // - OU qualquer PDF com ratio < 0.5% (muito pouco texto)
    const shouldForceOCR = (isLargePDF && textToFileSizeRatio < 0.01) || textToFileSizeRatio < 0.005;
...
async function extractRTF(filePath, sizeMB) {
  ...
  try {
    ...
    try {
      // 🚀 OTIMIZADO: spawnAsync não bloqueia Node.js
      const output = await spawnAsync('pandoc', ['-f', 'rtf', '-t', 'plain', filePath], {
        maxBuffer,
        timeout: 1800000
      });
      STATS.toolsUsed.add('pandoc');
...
export async function extractDocument(filePath) {
  ...
  switch (ext) {
    ...
    case '.odt':
      ...
      try {
        // 🚀 OTIMIZADO: spawnAsync não bloqueia Node.js
        const output = await spawnAsync('pandoc', ['-f', 'odt', '-t', 'plain', filePath], {
          maxBuffer,
          timeout: 1800000
        });
