...
export class DocumentProcessorV2 {
  ...
  tryRepairJSON(jsonString) {
    try {
      ...
      console.log(`   ⚠️  JSON parsing falhou: ${e.message}`);
      console.log(`   🔧 Tentando reparar JSON truncado...`);

      // Strategy 1: Fechar strings abertas (unterminated string error)
      if (e.message.includes('Unterminated string') || e.message.includes('unterminated string')) {
        try {
          // Contar aspas duplas não-escapadas para detectar string aberta
          let quoteCount = 0;
          let escaped = false;
          ...
          // Se número ímpar de aspas, string está aberta - fechar no final
          if (quoteCount % 2 !== 0) {
            // Adicionar aspa de fechamento no final do JSON truncado
            let withClosedString = jsonString + '"';

            // Tentar fechar o objeto também
            const openBraces = (withClosedString.match(/\{/g) || []).length;
            const closeBraces = (withClosedString.match(/\}/g) || []).length;

            if (openBraces > closeBraces) {
              withClosedString += '}';
            }

            const parsed = JSON.parse(withClosedString);
            console.log(`   ✅ JSON reparado: string fechada (${quoteCount} aspas → ${quoteCount + 1})`);
            return parsed;
          }
        } catch (e2) {
          console.log(`   ⚠️  Tentativa 1 falhou: ${e2.message}`);
        }
      }

      // Strategy 2: Truncar no último } válido (caso original)
      try {
        const lastBrace = jsonString.lastIndexOf('}');
        if (lastBrace > 0) {
          const truncated = jsonString.substring(0, lastBrace + 1);
          const repaired = JSON.parse(truncated);
          console.log(`   ✅ JSON reparado: truncado em ${lastBrace}`);
          return repaired;
        }
      } catch (e2) {
        console.log(`   ⚠️  Tentativa 2 falhou: ${e2.message}`);
      }

      // Strategy 3: Tentar encontrar primeiro objeto válido
      try {
        ...
        if (firstBrace >= 0 && lastBrace > firstBrace) {
          ...
          console.log(`   ✅ JSON reparado: extraído objeto de ${firstBrace} a ${lastBrace}`);
          return parsed;
        }
      } catch (e3) {
        console.log(`   ⚠️  Tentativa 3 falhou: ${e3.message}`);
      }

      console.log(`   ❌ Todas as tentativas de reparo falharam`);
      console.log(`   📊 JSON length: ${jsonString.length}, preview: ${jsonString.substring(0, 200)}...`);
  ...
  async extractWithChunking(rawText, documentId, documentName, jobId = null) {
    // 🔥 FIX: Reduzido de 400k para 200k chars
    // Motivo: 400k = 100k tokens + prompt ~10k = excede limite de 128k do Nova Micro
    // Novo: 200k = 50k tokens + prompt ~10k = 60k tokens (seguro, com margem)
    const MAX_CHUNK_SIZE = 200000; // 200k chars = ~50k tokens (seguro para Nova Micro + prompt)
...
`;
    ...
    try {
      ...
      // 🚨 VALIDAÇÃO CRÍTICA: Verificar se extração retornou texto válido
      if (!extractedText || extractedText.length === 0) {
        console.error(`   ❌ ERRO: Extração retornou texto vazio!`);
        throw new Error('Extração falhou: LLM retornou resposta vazia');
      }
      ...
      console.log(`   📊 Input: ${Math.round(rawText.length / 1000)}k chars`);
      console.log(`   📊 Output: ${Math.round(extractedText.length / 1000)}k chars`);
      console.log(`   📊 Compressão: ${Math.round((extractedText.length / rawText.length) * 100)}%`);
      console.log(`   💰 Custo: $${cost.toFixed(4)}`);

      // ⚠️ AVISO: Extração resultou em texto muito pequeno
      if (extractedText.length < rawText.length * 0.1) {
        console.warn(`   ⚠️  AVISO: Extração gerou menos de 10% do tamanho original`);
        console.warn(`   ⚠️  Isso pode indicar problema na extração`);
      }

      // ⚠️ AVISO: Output muito pequeno em geral
      if (extractedText.length < 5000) {
        console.warn(`   ⚠️  AVISO: Texto extraído muito pequeno (${extractedText.length} chars)`);
      }
  ...
  async saveExtractedTextToKB(extractedText, documentId, documentName, saveToDocuments = true, userId = null) {
    ...
    console.log(`   📊 Tamanho do texto a salvar: ${Math.round(extractedText.length / 1000)}k caracteres`);

    // 🚨 VALIDAÇÃO CRÍTICA: Verificar se texto extraído é válido
    if (!extractedText || extractedText.length === 0) {
      console.error(`   ❌ ERRO CRÍTICO: extractedText está vazio!`);
      throw new Error('Texto extraído está vazio. Não é possível salvar no KB.');
    }

    // ⚠️ AVISO: Texto muito pequeno pode indicar extração incompleta
    if (extractedText.length < 1000) {
      console.warn(`   ⚠️  AVISO: Texto extraído muito pequeno (${extractedText.length} chars)`);
      console.warn(`   ⚠️  Isso pode indicar extração incompleta ou documento vazio`);
    }

    // 📊 INFO: Texto muito grande - avisar sobre chunking
    if (extractedText.length > 5000000) {
      console.warn(`   ⚠️  AVISO: Texto muito grande (${Math.round(extractedText.length / 1000000)}MB)`);
      console.warn(`   ⚠️  Arquivos > 5MB podem levar muito tempo para processar`);
    }
    ...
    if (saveToDocuments) {
      ...
      if (!fs.existsSync(kbDocsDir)) {
        ...
        console.log(`   📁 Diretório criado: ${kbDocsDir}`);
      }

      // Salvar como TXT (não MD) para facilitar acesso
      const txtPath = path.join(kbDocsDir, `${timestamp}_00_TEXTO_COMPLETO.txt`);

      console.log(`   💾 Salvando 00_TEXTO_COMPLETO.txt...`);
      console.log(`   📂 Path: ${txtPath}`);
      console.log(`   📊 Tamanho: ${Math.round(extractedText.length / 1024)}KB`);

      try {
        fs.writeFileSync(txtPath, extractedText, 'utf-8');
        console.log(`   ✅ Arquivo físico salvo com sucesso`);
      } catch (writeError) {
        console.error(`   ❌ ERRO ao salvar arquivo físico: ${writeError.message}`);
        throw writeError;
      }

      // Verificar se arquivo foi realmente criado
      if (!fs.existsSync(txtPath)) {
        console.error(`   ❌ ERRO: Arquivo não foi criado! Path: ${txtPath}`);
        throw new Error(`Falha ao criar arquivo: ${txtPath}`);
      }

      const actualSize = fs.statSync(txtPath).size;
      console.log(`   ✓ Arquivo verificado no disco: ${actualSize} bytes`);
      ...
      console.log(`   💾 Adicionando ao KB cache...`);
      console.log(`   🆔 Doc ID: ${txtDoc.id}`);
      console.log(`   👤 User ID: ${txtDoc.userId}`);

      try {
        kbCache.add(txtDoc);
        console.log(`   ✅ Adicionado ao KB cache com sucesso`);
      } catch (cacheError) {
        console.error(`   ❌ ERRO ao adicionar ao cache: ${cacheError.message}`);
        // Continue - arquivo foi salvo, cache pode ser reconstruído depois
      }
      ...
      console.log(`   🆔 Doc ID: ${txtDoc.id}`);
      console.log(`   ✅ 00_TEXTO_COMPLETO.txt criado com sucesso!`);
    }
  ...
  async saveTechnicalFilesToKB(technicalFiles, documentId, documentName, intermediateDocId, userId = null) {
    ...
    for (const [fileKey, fileContent] of Object.entries(technicalFiles)) {
      ...
      // Placeholders têm ~150 bytes, fichamentos válidos devem ter > 500 bytes
      const MIN_VALID_SIZE = 500;
      if (fileContent.length < MIN_VALID_SIZE) {
        console.warn(`   ⚠️  SKIP: ${fileKey} muito pequeno (${fileContent.length} bytes < ${MIN_VALID_SIZE}), provavelmente placeholder`);
        console.warn(`   📄 Preview: ${fileContent.substring(0, 100)}...`);
        continue;  // Pular salvamento de fichamentos vazios/inválidos
      }
      ...
      // Salva conteúdo (apenas se passou validação)
   ...
   */
  async analyzeWithPremiumLLM(extractedText, analysisPrompt, model = 'sonnet', systemPrompt = '', timeoutMs = 300000) {
...
`;
  ...
  async generateTechnicalFilesBatch(extractedText, documentId, documentName, model = 'sonnet', progressCallback = null) {
    ...
    let timeoutMs = 300000; // 🔥 FIX: 5 minutos por batch (era 2min - insuficiente)
...
`;
    ...
    const fichamento = await this.analyzeWithPremiumLLM(extractedText, fichamentoPrompt, model, 'Você é um assistente especializado em fichamento de processos judiciais.', 300000);  // 🔥 FIX: 5 min timeout (era 2min - insuficiente)
...
`;
    ...
    const analise = await this.analyzeWithPremiumLLM(extractedText, analisePrompt, model, 'Você é um advogado sênior especializado em análise crítica de peças processuais.', 300000);  // 🔥 FIX: 5 min timeout (era 2min - insuficiente)
...
`;
    ...
    const cronologia = await this.analyzeWithPremiumLLM(extractedText, cronologiaPrompt, model, 'Você é um assistente especializado em análise temporal de processos.', 300000);  // 🔥 FIX: 5 min timeout (era 2min - insuficiente)
...
`;
    ...
    const resumo = await this.analyzeWithPremiumLLM(extractedText, resumoPrompt, model, 'Você é um analista que cria resumos executivos para advogados sêniores.', 300000);  // 🔥 FIX: 5 min timeout (era 2min - insuficiente)
  ...
  async processComplete(rawText, documentId, documentName, options = {}) {
    ...
    try {
      ...
      // 🔥 FIX CRÍTICO: PULAR extração com IA para PDFs (já vem limpo do pdf-parse)
      if (options.skipExtraction) {
        console.log(`\n✅ [ETAPA 1] Pulando extração com IA (PDF já limpo)`);
        console.log(`   📊 Usando texto do pdf-parse diretamente: ${Math.round(rawText.length/1000)}k chars`);

        extraction = {
          extractedText: rawText,  // Usar texto original do PDF
          metadata: {
            model: 'pdf-parse-direct',
            cost: 0,
            processingTime: 0,
            originalSize: rawText.length,
            extractedSize: rawText.length,
            method: 'skip-extraction-pdf'
          }
        };

        if (progressCallback) {
          await progressCallback('extraction', 15, 'Texto extraído diretamente do PDF');
        }
      } else {
        // Extração normal com IA para documentos não-PDF
        if (progressCallback) {
        ...
        extraction = await this.extractFullText(rawText, documentId, documentName);
