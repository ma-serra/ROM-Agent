...
class PromptsAutoMigrate {
   ...
   */
  getPromptsDirectory() {
    // 🔧 v3.6.1: SEMPRE usar caminho relativo seguro do projeto
    // Render não tem /var/data com permissões adequadas
    // Usar data/prompts/ dentro do projeto (ephemeral OK para prompts)
    return this.sourceDir;

    // NOTA: Código antigo tentava usar /var/data/prompts mas causava ENOENT no Render
    // Prompts são recriados a cada deploy (comportamento esperado)
  }
   ...
   */
  async migrate() {
    // 🔧 v3.6.1: DESABILITADO - Sempre usar data/prompts/ local do projeto
    // Migração para /var/data causava ENOENT no Render
    console.log('📝 [Prompts] Usando data/prompts/ do projeto (ephemeral safe)');
    return { success: true, migrated: false, reason: 'using-project-dir' };

    /* CÓDIGO ANTIGO - REMOVIDO POR CAUSAR ENOENT
